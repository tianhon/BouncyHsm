...
[TestClass]
public class T20_VerifyCamellia
{
    ...
    [DataRow(24)]
    [DataRow(32)]
    public void Verify_CamellicaMac_Success(int size)
    {
        ...
        IObjectHandle handle = this.GenerateCamelliaKey(session, size);
        using IMechanism mechanism = factories.MechanismFactory.Create(CKM.CKM_CAMELLIA_MAC);
    ...
    [DataRow(24, 4)]
    [DataRow(32, 16)]
    public void Verify_CamelliaMacGeneral_Success(int size, int parameter)
    {
        byte[] dataToSign = new byte[64];
        Random.Shared.NextBytes(dataToSign);
        ...
        using IMechanism mechanism = factories.MechanismFactory.Create(CKM.CKM_CAMELLIA_MAC_GENERAL, mechanismParam);
    ...
    private IObjectHandle GenerateCamelliaKey(ISession session, int size)
    {
